...
# Task
深度解析 prd.md，将所有需求点以模块化聚合的形式挂载到 UI 页面。根据用户的明确指令，选择并执行以下两个工作流之一。核心指标：研发人员通过浮窗获取的信息必须足够详实，足以完全替代阅读原始 PRD 文档。
...
- **【顶部标题栏】**：需求编号 + “需求描述：[模块名称]”。下方有一条浅灰色分割线。需求编号的样式与角标样式要完全一致。
- **【Markdown 深度渲染】**：
  - 段落与间距：严格保留原文的段落结构。行高设定为 1.6，段落间距（Margin-bottom）设定为 12px，确保长文不拥挤。
  - 重点突出：必须 1:1 还原原文档中的加粗 和 斜体，用于强调核心逻辑和关键字。
...
- **层级管理**：
  ...
  - 浮窗层级： `z-index: 9959;`（确保在最顶层）。
- **智能避让与边界**： 浮窗默认出现在角标的左下方（间距 3px）。若临近页面边缘导致溢出视口，需自动反向调整位置。
...
- **角标样式（严格执行）**：
  ...
  - 编号使用数字 1-999。
- **浮窗样式**：
  - 背景色 `#f0efef`，`border-radius: 4px;`，宽度 450px，带有轻微弥散阴影，右上角含“X”关闭按钮。
  - 多浮窗与交互逻辑：
...
- **反向注入**： 将生成的 1-999 需求编号反向写入 prd.md 对应需求描述起始位置（格式：`[1]`）。
- **逻辑同步**： 确保页面标注编号与文档内编号严格一一对应。
- **需求编号格式**： 写入 prd.md 的需求编号的样式与角标样式要完全一致。
...
- **样式自检**： 角标是否符合 10px 粗体规范？层级是否正确（不遮挡底层，但浮窗在最顶层）？浮窗是否完美保留了原文的 Markdown 层级与重点？
- **样式锁自检**： 无论内容如何变化，视觉参数（颜色、尺寸、偏移量）是否保持了 100% 的一致性？
